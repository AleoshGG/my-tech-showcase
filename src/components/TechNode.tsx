...
interface TechNodeProps {
  name: string;
  /** Pass a real icon/image src here */
  iconSrc?: string;
  /** Fallback initials when no icon */
  initials?: string;
  x: number;
  y: number;
  delay?: number;
...
const TechNode = ({ name, iconSrc, initials, x, y, delay = 0 }: TechNodeProps) => (
  <motion.div
  ...
  >
    <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl border border-[hsl(var(--surface-glass)/0.1)] bg-[hsl(var(--surface-glass)/0.05)] flex items-center justify-center overflow-hidden backdrop-blur-sm hover:border-primary/50 transition-colors duration-300 shadow-[0_0_0_1px_rgba(255,255,255,0.05),0_10px_30px_-10px_rgba(0,0,0,0.5)]">
      {iconSrc ? (
        <img
          src={iconSrc}
          alt={name}
    ...
    {/* Tooltip popup */}
    <div className="absolute -top-12 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none scale-90 group-hover:scale-100">
